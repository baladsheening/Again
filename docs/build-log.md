# The build log — 8 September and earlier
...
⚠ **That sentence said *7 September onward* until the fifth cut and is corrected
rather than left:** the newer entries — **11 September onward** — stay in `CLAUDE.md` under *Where the
build stands*.
⚠⚠ **A FIFTH CUT — 12 September, at the 8 September boundary, AND THIS ONE WAS
MADE WITH THE FILE ALREADY OVER.** `CLAUDE.md` reached **150,741** against the
150,000 the harness loads — the overflow was not predicted and then avoided, it
had already happened, and a file over the limit is silently truncated from the
**oldest** end. Everything dated **8 September** moved here in one block, in
order and verbatim: `black-translucent` and the 46px the installed app was
throwing away, the keyboard-only rearrangement, the ink leaving before the slab,
the bar's ink fade, the composer's foot row learning to close, and the rename to
**JUCE** with the six wordmark numbers it moved. ⚠ **Not superseded by the
move**; every one of them describes live behaviour and **every ⚠ in it still
binds** — the `black-translucent` tombstone and the `writing-sheet` in-flow
tombstone above all.

⚠ **What stays in `CLAUDE.md` is 11 September onward**, plus the standing
register under *Where the build stands*.
