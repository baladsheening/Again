...
Status: historical convergence implementation register. Its privacy and
interaction reasoning remains available, but the Release 1 consumer requirements
now come from `consumer-product-strategy.md` and Amendment 10 to
...
| ⚠ The record moves to its own surface | **DIRECTED 5 September, NOT BUILT** — Amendment 5. The console, the lock swipe, the mark, the fade and the portal's render prop **travel with it and none is rewritten**; only the route changes. `docs/re-direction/inactive/the-front-page.md`. ⚠ The foot returns to three columns and **the `+` goes away**, so the portal door's placement — granted the foot against §2 partly because column one was empty — wants re-reading rather than inheriting |
